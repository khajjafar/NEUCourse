...
That was the problem we set out to fix with NEUCourse - a degree planning and scheduling tool built specifically for NEU students. Keeyon and Joy built it as a course project, two engineers over two one-week sprints, and the experience taught us more about software architecture, AI-assisted development, and the gap between "we could build that" and "we should build that" than any single project we've worked on before.
