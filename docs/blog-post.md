# Building NEUCourse: Two Sprints, Three AI Tools, and One Very Chaotic Registration Season
...
## Three AI Tools, Three Very Different Jobs
...
The primary coding tool was VS Code AI through the Antigravity extension. This was the workhorse — it handled feature implementation prompts, generated boilerplate for new API routes, and filled in the repetitive parts of components. When I needed to add a new hook that followed the same pattern as `usePlans`, I could describe what I wanted and get a working scaffold in seconds. This is where AI adds the most raw throughput: when you know exactly what you want and you just need someone to type it.

Claude Code CLI — the tool I'm writing this in — played a different role. I used it for code review and architectural guidance, for writing the test suite, and for documentation like this blog post. The strength here is multi-file awareness: being able to reason across the whole codebase, spot inconsistencies between a route handler and its test, or notice that a component is importing something it shouldn't. When I was debugging the en-dash issue in `parse-meeting-times.ts`, Claude Code helped me trace the call chain from the calendar button all the way back to the scraper output format. That kind of cross-file reasoning is where a tool with broad context beats an inline autocomplete.

Claude Chat was the third tool, mostly used in earlier planning stages. When Keeyon and I were figuring out how to structure the Firestore data model — should semesters be a subcollection or an array on the plan document? — I used Claude Chat to think through the trade-offs out loud. Subcollections scale better and allow security rules per document. Arrays are simpler to read in one roundtrip. We went with subcollections. That conversation shaped a lot of downstream decisions about the API design.

The three tools genuinely complemented each other: Claude Chat for planning and design, Antigravity for implementation, Claude Code for review and quality. Using all three well required knowing which one to reach for — and being disciplined enough not to let any of them override your own judgment when the suggestion was wrong.
